...
func TestThresholdValidator2of3_Mekatek(t *testing.T) {
	total := uint8(3)
	threshold := uint8(2)
	...
	challenge := &mekabuild.RegisterChallenge{Bytes: []byte("foobarbaz")}

	signBytes := challenge.SignableBytes()

	err = validator.SignMekatekRegisterChallenge(challenge)
	if err != nil {
		t.Logf("%v", err)
	}
	require.NoError(t, err)

	require.True(t, privateKey.PubKey().VerifySignature(signBytes, challenge.Signature))
	require.False(t, privateKey.PubKey().VerifySignature(signBytes, []byte("bogus")))

	buildReq := &mekabuild.BuildBlockRequest{
		ChainID:          "foobar",
		Height:           1,
		ValidatorAddress: "42",
		MaxBytes:         2,
		MaxGas:           3,
		Txs:              [][]byte{[]byte(`sending coins`)},
	}

	signBytes = buildReq.SignableBytes()

	err = validator.SignMekatekBuildBlockRequest(buildReq)
	if err != nil {
		t.Logf("%v", err)
	}
	require.NoError(t, err)

	require.True(t, privateKey.PubKey().VerifySignature(signBytes, buildReq.Signature))
	require.False(t, privateKey.PubKey().VerifySignature(signBytes, []byte("bogus")))
}
